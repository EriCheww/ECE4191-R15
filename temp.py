...
class AnnotatorApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        open_btn = ctk.CTkButton(sidebar, text="Open Image", command=self._open_image)
        open_btn.pack(padx=(10,10), pady=(10, 0), fill="x")
    ...
    def _open_image(self):
        path = filedialog.askopenfilename(
            ...
            filetypes=[("Images", "*.png;*.jpg;*.jpeg;*.bmp;*.webp;*.tif;*.tiff"), ("All", "*.*")]
        )
        if not path:
            return
        try:
    ...
    def _set_photo(self, img: Image.Image):
        ...
        self.canvas.config(width=min(1600, img.width), height=min(1200, img.height))
    ...
    def _layout_handles(self, box: Box):
        ...
        for hid, (x, y) in zip(self.handle_ids, pts):
            self.canvas.coords(hid, x - s, y - s, x + s, y + s)
    ...
    def _redraw_fast(self):
        if self.img_orig:
            ...
            self.canvas.coords(self.crop_id, x1, y1, x2, y2)
        ...
        for i, (rid, tid) in enumerate(self.box_item_ids):
            if i < len(self.boxes):
                ...
                self.canvas.coords(tid, bx1 + 4, max(0, by1 - 6))
        ...
        if self.temp_box is not None and self.mode.get() == "box":
            x1, y1, x2, y2 = self.temp_box.normalized_xyxy()
            self.canvas.coords(self.temp_id, x1, y1, x2, y2)
    ...
    def _hit_handle(self, box: Box, x: float, y: float) -> Optional[str]:
        ...
        cx, cy = (x1 + x2) / 2, (y1 + y2) / 2
        regions = {
            "nw": (x1 - s, y1 - s, x1 + s, y1 + s),
            "ne": (x2 - s, y1 - s, x2 + s, y1 + s),
            "sw": (x1 - s, y2 - s, x1 + s, y2 + s),
            "se": (x2 - s, y2 - s, x2 + s, y2 + s),
            "n": (cx - s, y1 - s, cx + s, y1 + s),
            "s": (cx - s, y2 - s, cx + s, y2 + s),
            "w": (x1 - s, cy - s, x1 + s, cy + s),
            "e": (x2 - s, cy - s, x2 + s, cy + s),
        }
    ...
    def _on_motion(self, evt):
        ...
        m = self.mode.get()
        if m == "crop":
            h = self._hit_handle(self.crop, x, y)
            if h:
                cur = self._cursor_for_handle(h)
            elif self._hit_inside(self.crop, x, y):
                cur = "fleur"
        elif m == "box":
            idx = self._hit_box(x, y)
            if idx is not None:
                h = self._hit_handle(self.boxes[idx], x, y)
                if h:
                    cur = self._cursor_for_handle(h)
                else:
                    cur = "fleur"
    ...
    def _on_press(self, evt):
        ...
        if m == "crop":
            ...
            if idx is not None:
                ...
                elif self._hit_inside(self.boxes[idx], x, y):
                    self.drag_target = ("box-move", idx)
                    self.selected_idx = idx
                    self._request_redraw()
            else:
                self.temp_box = Box(x, y, x, y, "")
    ...
    def _on_drag(self, evt):
        ...
        if kind == "crop-move":
            ...
            self.crop.resize_from_handle(handle, x, y, (self.W, self.H))
            ...
            self.boxes[idx].resize_from_handle(handle, x, y, (self.W, self.H))

        elif kind == "new-box":
            if self.temp_box is not None:
                self.temp_box.x2, self.temp_box.y2 = x, y
...
def advanced_screenshot_from_widget(widget):
    """
    Capture the given Tk widget and open AnnotatorApp with the image preloaded.

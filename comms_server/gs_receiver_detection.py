...
"""
gs_relay_stream.py Gstreamer relay for webRTC viewing of Pi Camera Stream.

Takes the RTP/H264 UDP stream from the Raspberry Pi sender and relays it via a webrtcsink
element with built-in signalling server.

gui.py provides a GUI to view the stream and take screenshots.
"""
...
WEB_DIR = r"C:\Users\freem\OneDrive\Documents\GitHub\ECE4191-R15\comms_server\webrtcsink-webui"
...
pipeline = (
    ...
    # --- Branch A: WebRTC viewer (passthrough) ---
    f't. ! queue max-size-buffers=1 max-size-time=0 max-size-bytes=0 leaky=downstream '
    f'   ! h264parse config-interval=-1 '
    f'   ! video/x-h264,stream-format=avc,alignment=au '
    f'   ! webrtcsink '
    f'       run-signalling-server=true run-web-server=false '
    f'       signalling-server-host=0.0.0.0 signalling-server-port={SIG_PORT} '
    f'       stun-server=stun://stun.l.google.com:19302 '
    f'       video-caps=video/x-h264,stream-format=avc,alignment=au '
    f'       meta="meta,name={STREAM_NAME}" '

    # --- Branch B: local loopback for detector (RTP/H264) ---
    f't. ! queue max-size-buffers=1 max-size-time=0 max-size-bytes=0 leaky=downstream '
    f'   ! h264parse config-interval=1 ! rtph264pay pt={PAYLOAD_PT} '
    f'   ! udpsink host=127.0.0.1 port=5600 sync=false async=false'
)
